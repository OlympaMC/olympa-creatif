...
/*
		...
		scm.addLines(
					...
					new TimerLine<OlympaPlayerCreatif>( p -> {			
						if (p.getCustomScoreboardLines().get(1) != null)
							return p.getCustomScoreboardLines().get(1);
						else {
					...
					new TimerLine<OlympaPlayerCreatif>( p -> {			
						if (p.getCustomScoreboardLines().get(2) != null)
							return p.getCustomScoreboardLines().get(2);
						else {
					...
					new TimerLine<OlympaPlayerCreatif>( p -> {			
						if (p.getCustomScoreboardLines().get(3) != null)
							return p.getCustomScoreboardLines().get(3);
						else
							return "§2";
					}, plugin, 20),
				
				new TimerLine<OlympaPlayerCreatif>( p -> {				
					if (p.getCustomScoreboardLines().get(4) != null)
						return p.getCustomScoreboardLines().get(4);
					else
						return "§7Grade : " + p.getGroupNameColored();
				}, plugin, 20),
	
				new TimerLine<OlympaPlayerCreatif>( p -> {			
					if (p.getCustomScoreboardLines().get(5) != null)
						return p.getCustomScoreboardLines().get(5);
					else {
						Plot plot = plugin.getPlotsManager().getPlot(p.getPlayer().getLocation());
						if (plot == null)
				...
				new TimerLine<OlympaPlayerCreatif>( p -> {				
					if (p.getCustomScoreboardLines().get(6) != null)
						return p.getCustomScoreboardLines().get(6);
					else
						return "§3";
				}, plugin, 20),
	
				new TimerLine<OlympaPlayerCreatif>( p -> {			
					if (p.getCustomScoreboardLines().get(7) != null)
						return p.getCustomScoreboardLines().get(7);
					else
						return "§7Kumars : §6" + p.getGameMoneyFormated();
				}, plugin, 20),
	
				new TimerLine<OlympaPlayerCreatif>( p -> {				
					if (p.getCustomScoreboardLines().get(8) != null)
						return p.getCustomScoreboardLines().get(8);
					else
						return "§7Omégas : §6" + p.getStoreMoney().getFormatted();
				}, plugin, 20)
		);
