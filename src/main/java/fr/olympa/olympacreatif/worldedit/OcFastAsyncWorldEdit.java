...
public class OcFastAsyncWorldEdit extends AWorldEditManager {
	...
	private class OlympaCreatifMask extends BukkitMaskManager {
		...
		public FaweMask getMask(final com.sk89q.worldedit.entity.Player wePlayer, MaskType type) {
			...
			Plot plot;
			if (p == null || (plot = plugin.getPlotsManager().getPlot(p.getPlayer().getLocation())) == null || !PlotPerm.USE_WE.has(plot, p) || !ComponentCreatif.WORLDEDIT.isActivated() || isReseting(plot)) {
